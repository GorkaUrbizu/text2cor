				...
				if "#begin document" in line:
					...
					for i in range(len(doc)):
						for j in range(i+1, len(doc)):
							...
							if 1: #j == i+4 or j >= len(doc)-1 or r <= 0.25:
					...
					if sentence and current_sentence != tab[1]:
						current_sentence = tab[1]
						if sentence[-1] not in [".","!","?"]: #if last token of sentence not a punctuation mark: add a dot
							sentence.append(".")
							cor.append("_")
						doc.append(sentence)
						coref.append(cor)
